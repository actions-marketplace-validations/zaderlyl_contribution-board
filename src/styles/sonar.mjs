// Style "sonar" : les 10 jours avec le plus de commits deviennent des
// émetteurs qui lancent un ping (cercle qui s'agrandit). Chaque autre jour
// actif est "détecté" (révélé) au moment précis où le ping de l'émetteur
// le plus proche l'atteint — plus un jour est loin de tout émetteur, plus
// il met de temps à apparaître.
...
export const meta = {
  id: "sonar",
  label: "Commit Sonar",
  description: "Les 10 plus gros jours de commits lancent un ping qui détecte tous les autres, de proche en proche.",
};
...
export function render(days, opts = {}) {
  ...
  const EPS = 0.05;
  const EXPAND_END = 0.72; // fin de la phase où les pings se propagent
  const HOLD_END = 0.92;   // pause, tout reste détecté, avant la boucle
  ...
  // (top 10) sont visibles dès le début, plus gros.
  let blipEls = "", blipKeyframes = "";
  activeDays.forEach((d, i) => {
    const [x, y] = pos(d);
    const isOrigin = originSet.has(d);
    const r = isOrigin ? 3 : 2;
    if (isOrigin) {
      blipEls += `<circle cx="${x}" cy="${y}" r="${r}" fill="${accent}"/>\n`;
      return;
    }
    const name = `blip${i}`;
    ...
    blipKeyframes += `@keyframes ${name} {\n` +
      ...
      `  100% { opacity: 1; }\n` +
      `}\n`;
    blipEls += `<circle cx="${x}" cy="${y}" r="${r}" fill="${accent}" style="animation: ${name} ${CYCLE}s linear infinite; opacity:0;"/>\n`;
  });

  // Anneaux : un cercle par émetteur, rayon 0 -> maxDist (même vitesse pour
  // tous), opacité qui décroît en s'éloignant pour l'effet "ping" qui
  // s'estompe.
  const ringKeyframes = `@keyframes ringGrow {
  0% { r: 0px; opacity: 0.9; }
  ${pct(EXPAND_END).toFixed(2)}% { r: ${maxDist.toFixed(1)}px; opacity: 0; }
  100% { r: ${maxDist.toFixed(1)}px; opacity: 0; }
}`;
  let ringEls = "";
  origins.forEach((o) => {
    ringEls += `<circle cx="${o.x}" cy="${o.y}" r="0" fill="none" stroke="${accent}" stroke-width="1.2" style="animation: ringGrow ${CYCLE}s linear infinite;"/>\n`;
  });
